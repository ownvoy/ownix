...
# Guía de Actualización ownix 2.3 → 2.4

Esta guía proporciona scripts de actualización seguros para migrar de ownix 2.3 a 2.4, manejando cambios incompatibles y preservando tu configuración personalizada.
...
- **Respaldo completo** de ~/ownix antes de cambios
...
```bash
# Puedes ejecutarlo desde cualquier ruta; opera sobre ~/ownix
...
- **Genera un informe detallado** en `~/ownix-upgrade-analysis-TIMESTAMP.txt`
...
1. Elimina `~/ownix`: `rm -rf ~/ownix`
2. Restaura desde el respaldo: `cp -r ~/.config/ownix-backups/[backup-name]/ownix ~/`
3. Rebuild: `nh os boot ~/ownix --hostname [profile]`
4. Reinicia
...
- Verifica tu terminal preferida en `~/ownix/hosts/[hostname]/variables.nix`
- Confirma que navegador y otras apps sigan disponibles

### Log de ejecución
Cada corrida genera un log en: `~/ownix-upgrade-TIMESTAMP.log`

## 📁 Estructura tras la actualización

```
~/ownix/hosts/[tu-hostname]/
...
## Archivos de Script
- `upgrade-ownix-2.3-to-2.4.sh` — Script principal
- `revert-ownix-to-2.3.sh` — Wrapper de reversión
