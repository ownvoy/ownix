...
pkgs.writeShellScriptBin "zcli" ''
  ...
  verify_hostname() {
    ...
    if [ -f "$FLAKE_NIX_PATH" ]; then
      if ! ${pkgs.gnugrep}/bin/grep -qE "^[[:space:]]*$current_hostname[[:space:]]*=[[:space:]]*\\{" "$FLAKE_NIX_PATH"; then
        echo "Error: Current hostname is not registered in $FLAKE_NIX_PATH" >&2
        echo "  Current hostname: '$current_hostname'" >&2
        echo "" >&2
        echo "Hint: Run 'zcli update-host $current_hostname <profile>' to register it." >&2
        exit 1
      fi
    else
  ...
  case "$1" in
    ...
    rebuild)
      ...
      if eval "${pkgs.nh}/bin/nh os switch --hostname '$(${pkgs.nettools}/bin/hostname)' $extra_args"; then
    ...
    rebuild-boot)
      ...
      if eval "${pkgs.nh}/bin/nh os boot --hostname '$(${pkgs.nettools}/bin/hostname)' $extra_args"; then
    ...
    update)
      ...
      if eval "${pkgs.nh}/bin/nh os switch --hostname '$(${pkgs.nettools}/bin/hostname)' --update $extra_args"; then
    ...
    update-host)
      ...
      if ${pkgs.gawk}/bin/awk \
        -v host="$target_hostname" \
        -v profile="$target_profile" \
        '
          BEGIN {
            inMachines = 0;
            inHost = 0;
        ...
        ' "$FLAKE_NIX_PATH" > "$FLAKE_NIX_PATH.tmp" && ${pkgs.coreutils}/bin/mv "$FLAKE_NIX_PATH.tmp" "$FLAKE_NIX_PATH"; then
        echo "Successfully registered host '$target_hostname' with profile '$target_profile'"
      else
        echo "Error: Failed to update machine registry in $FLAKE_NIX_PATH" >&2
        ${pkgs.coreutils}/bin/rm -f "$FLAKE_NIX_PATH.tmp"
        exit 1
      fi
        ...
        install)
          if ! is_doom_enabled; then
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
              ...
              if ${pkgs.nh}/bin/nh os switch --hostname "$(${pkgs.nettools}/bin/hostname)"; then
