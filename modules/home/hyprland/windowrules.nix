...
in {
  wayland.windowManager.hyprland = {
    settings = {
      # Hyprland 0.53+ window rule syntax: matchers are prefixed with `match:`
      # and written `match:<prop> <value>`; effects are `<effect> <value>`.
      # Boolean effects take `on`. Renames: noblur->no_blur,
      # keepaspectratio->keep_aspect_ratio, idleinhibit->idle_inhibit,
      # initialTitle->initial_title. Regex negation uses the `negative:` prefix.
      windowrule = [
        ...
        "opacity 0.82 0.82, match:class ^([Nn]eovide)$"
