...
your `ownix` environment. It provides a comprehensive set of commands to perform common
...
<summary>**❄ Why did you create ownix ? **</summary>
...
- The `ownix` name is an inside joke among friends.
...
- You create a fork of ownix, then modify it.
- If you find an issue and fix it, or provide a new feature, please share it.
- ownix is not a distro. At this time there are no plans to create an install
  ISO.
...
- Edit `~/ownix/modules/core/flatpak.nix`
...
1. In the file, `~/ownix/modules/core/system.nix`
...
Monitor settings are in the file: `~/ownix/hosts/<HOSTNAME>/variables.nix`
...
Followed by a list of packages These are required for ownix.
...
<details>
<summary>** I made a change to my ownix configuration, how do I activate it? **</summary>
...
1. You have to edit the `hardware.nix` file for that host in
   `~/ownix/hosts/HOSTNAME/hardware.nix` and override the default.
...
2. Edit the `~/ownix/flake.nix` file. Change the line:

- `host = "NEW-HOSTNAME"`

3. In the `~/ownix` Directory run `git add .` _The rebuild will fail with a
...
1. Edit the `~/ownix/modules/core/boot.nix` file.
2. Look for:
...
1. Either run the `install-ownix.sh` script and select `nvidia-laptop`
   template or if configuring manually, set the template in the `flake.nix` to
   `nvidia-prime`

2. In the `~/ownix/hosts/HYBRID-HOST/variables.nix` file you will need to set
...
4. In the `~/ownix/modules/home/hyprland/config.nix` file is an ENV
...
1. Edit the `~/ownix/modules/core/stylix.nix` file.
...
1. Edit the `~/ownix/hosts/HOSTNAME/varibles.nix`
...
- Wallpapers are stored in the `~/ownix/wallpapers` directory.
...
1. Edit the `~/ownix/modules/home/hyprland/config.nix` file.
...
1. Edit the `~/ownix/modules/home/scripts/wallsetter`
...
<details>
<summary>**⬆ How do I update ownix?  **</summary>
...
1. First backup your existing `ownix` directory. e.g.
...
3. You need to install ownix like a new install. `./install-ownix.sh`
...
2. Backup your `ownix` directory e.g. `cp -r ~/ownix ~/ownix-backup`

3. Run the `./install-ownix.sh` script and follow the new install
   instructions.
...
<details><summary>**📂 ownix v2.3 Layout**</summary>
...
~/ownix/
    ...
    ├── install-ownix.sh          # Install script for ownix
    ├── LICENSE                     # MIT license ownix is using
    └── README.md                   # Intro document for ownix
```
...
- If you find other files that cause this you can add them in the
  `ownix/modules/home/scripts/default.nix`
  <br>
...
The Yazi configuration file is located in `~/ownix/modules/home/yazi.nix`

Yazi is configured like VIM and VIM motions

The keymap is in the `~/ownix/modules/home/yazi/keymap.toml` file
...
- That feature is called "cursor_trail" in the
  `~/ownix/modules/home/kitty.nix` file.
...
The kitty bindings are configured in `~/ownix/modules/home/kitty.nix`
...
Edit the `/ownix/modules/home/wezterm.nix` Change `enable = false` to
`enable = true;`\
Save the file and rebuild ownix with the `fr` command.
...
The kitty bindings are configured in `~/ownix/modules/home/wezterm.nix`
...
1. Edit the `~/ownix/modules/home/ghostty.nix` file.
...
- Use the command `zcli rebuild` or `fr` Flake Rebuild alias. If you **created a
  ...
  `ownix` folder. If successful, a new generation will be generated with your
...
Edit the `/ownix/modules/home/wezterm.nix` Change `enable = false` to
`enable = true;`\
Save the file and rebuild `ownix` with the `zcli rebild` command or the `fr`
alias
