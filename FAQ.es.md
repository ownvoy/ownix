...
# 💬 Preguntas frecuentes de ownix para v2.4

- **Revisión v1.26**
- **Fecha:** 1-septiembre-2025

> IMPORTANTE: ¿Actualizando de v2.3 a v2.4?
> - Altamente recomendado: Lee `ownix-Upgrade.md` completamente antes de intentar la actualización.
...
La utilidad `zcli` es una herramienta de línea de comandos diseñada para simplificar la gestión de tu entorno `ownix`. Proporciona un conjunto completo de comandos para realizar tareas comunes como actualizar tu sistema, gestionar hosts, limpiar generaciones antiguas y gestionar Doom Emacs.
...
ownix CLI Utility -- version 1.0.2
...
<summary>**❄ ¿Por qué creaste ownix? **</summary>
...
- El nombre `ownix` es un chiste interno entre amigos.
...
- Crea un fork de ownix, luego modifícalo.
- Si encuentras un issue y lo arreglas, o proporcionas una nueva característica, por favor compártelo.
- ownix no es una distro. En este momento no hay planes para crear un ISO de instalación.
...
- Edita `~/ownix/modules/core/flatpak.nix`
...
- Ve al directorio `~/ownix/host/HOSTNAME`
...
1. En el archivo, `~/ownix/modules/core/system.nix`
...
La configuración del monitor está en el archivo: `~/ownix/hosts/<HOSTNAME>/variables.nix`
...
<img align="center" width="90%" src="https://github.com/ownvoy/ownix/raw/main/img/nwg-displays.png" />
...
<details>
<summary>**¿Cómo agrego aplicaciones a ownix? **</summary>

### Hay dos opciones. Una para todos los hosts que tienes, otra para un host específico.

1. Para aplicaciones que se incluirán en todos los hosts definidos edita el archivo `~/ownix/modules/core/packages.nix` file.

Hay una sección que comienza con: `environment.systemPackages = with pkgs;`

Seguida de una lista de paquetes. Estos son requeridos para ownix.
...
`~/ownix/hosts/<HOSTNAME>/host-packages.nix`
...
<details>
<summary>** Hice un cambio a mi configuración de ownix, ¿cómo lo activo? **</summary>

- Usa la utilidad `zcli`. `zcli rebuild`
- El alias legacy `fr`, Flake Rebuild está depreciado pero aún disponible **
  NOTA: Si **creaste un nuevo archivo**
- necesitarás ejecutar un comando `git add .` en la carpeta `ownix`
...
1. Tienes que editar el archivo `hardware.nix` para ese host en `~/ownix/hosts/HOSTNAME/hardware.nix` y override el default.
...
- `cp -rpv ~/ownix/hosts/OLD-HOSTNAME ~/ownix/hosts/NEW-HOSTNAME`

2. Edita el archivo `~/ownix/flake.nix`. Cambia la línea:

- `host = "NEW-HOSTNAME"`

3. En el directorio `~/ownix` ejecuta `git add .` _El rebuild fallará con un error 'file not found' si olvidas este paso._
...
1. Edita el archivo `~/ownix/modules/core/boot.nix` file.
2. Busca:
...
1. Ejecuta el script `install-ownix.sh` y selecciona el template `nvidia-laptop` o si configurando manualmente, establece el template en el `flake.nix` a `nvidia-prime`

2. En el archivo `~/ownix/hosts/HYBRID-HOST/variables.nix` necesitarás establecer los PCI IDs para las GPUs Intel y NVIDIA. Refiérete a [esta página](https://nixos.wiki/wiki/Nvidia) para ayudar a determinar esos valores.

3. Una vez que todo esté configurado apropiadamente, usa el alias Flake Rebuild `fr` para crear una nueva generación.

4. En el archivo `~/ownix/modules/home/hyprland/config.nix` hay un ENV setting`"AQ_DRM_DEVICES,/dev/dri/card0:/dev/dri/card1:/dev/dri/card2"` Esto establece la GPU primaria y secundaria. Usando la info del weblink arriba podrías tener que cambiar el orden de estos valores.
...
1. Edita el archivo `~/ownix/modules/core/stylix.nix` file.
...
1. Edita el archivo `~/ownix/hosts/HOSTNAME/varibles.nix`
2. Cambia el `stylixImage =` al nombre del archivo que quieres usar. Los wallpapers están en `~/ownix/wallpapers`
...
- Los wallpapers se almacenan en el directorio `~/ownix/wallpapers`.
...
1. Edita el archivo `~/ownix/modules/home/hyprland/config.nix` file.
...
1. Edita el archivo `~/ownix/modules/home/scripts/wallsetter`
...
<details>
<summary>**⬆ ¿Cómo actualizo ownix?  **</summary>
...
Altamente recomendado: Lee `ownix-Upgrade.md` antes de proceder. Detalla el upgrade automatizado seguro, respaldo, y proceso de revert.

Usa el upgrade automatizado v2.3 → v2.4. Ve `ownix-Upgrade.md` y `UPGRADE-2.3-to-2.4.md`. Para evitar sobrescribir tu config antes de crear un respaldo, fetch solo el script sin modificar tu working tree:
...
1. Primero respaldo tu directorio `ownix` existente. ej.
   `cp -r ~/ownix ~/ownix-backup`

2. No hay update directo. Cuando clones la nueva config los archivos de config y layout han cambiado.

3. Necesitas instalar ownix como una nueva install. `./install-ownix.sh`
...
2. Respalda tu directorio `ownix` ej. `cp -r ~/ownix ~/ownix-backup`

3. Ejecuta el script `./install-ownix.sh` y sigue las nuevas instrucciones de install.

</details>

<details>
<summary> ¿Cómo sé cuándo se lanza una nueva versión de ownix? </summary>
...
<details><summary>**📂 Layout de ownix v2.3**</summary>
...
~/ownix/
    ...
    ├── flake.nix                   # flake que controla config ownix
    ├── install-ownix.sh          # Script de install para ownix
    ├── LICENSE                     # Licencia MIT que usa ownix
    └── README.md                   # Documento introductorio para ownix
```
...
- Si encuentras otros archivos que causan esto puedes agregarlos en el `ownix/modules/home/scripts/default.nix`
  <br>
...
El archivo de configuración de Yazi está localizado en `~/ownix/modules/home/yazi.nix`

Yazi está configurado como VIM y motions VIM

El keymap está en el archivo `~/ownix/modules/home/yazi/keymap.toml` file
...
- Esa característica se llama "cursor_trail" en el archivo `~/ownix/modules/home/kitty.nix`.
...
Los bindings de kitty están configurados en `~/ownix/modules/home/kitty.nix`
...
Edita el `/ownix/modules/home/wezterm.nix` Cambia `enable = false` a `enable = true;`\
Guarda el archivo y rebuild ownix con el comando `zcli rebild` o el alias `fr`
...
Los bindings de kitty están configurados en `~/ownix/modules/home/wezterm.nix`
...
1. Edita el archivo `~/ownix/modules/home/ghostty.nix` file.
