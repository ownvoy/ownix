...
- Repo location is expected at ~/ownix. If you place it elsewhere, update modules/core/nh.nix (programs.nh.flake) to the new path.

Quick commands
- Build and switch (preferred: nh/zcli)
  - fr  → nh os switch --hostname <hostname>   # alias provided by zsh/bash modules
...
- Host management
  - zcli update-host [hostname] [profile]   # registers or updates machines.<hostname>.profile
...
- Choose by setting the host's configured profile in `flake.nix`; build with the host target (e.g., `.#my-desktop`)

High-level architecture
- flake.nix
  - Inputs: nixpkgs 25.05, home-manager 25.05, stylix, nvf, nix-flatpak
  - Local constants: system, username, machines
  - nixosConfigurations: one per host via mkNixosConfig
    - Each host configuration imports profiles/<profile> based on `machines.<hostname>.profile`
...
- modules/core
  ...
  - nh.nix enables nh, configures GC, and pins programs.nh.flake = /home/${username}/ownix
- modules/drivers
...
- flake.nix: set username; register hosts under `machines.<hostname>.profile`; add inputs; wire outputs
...
- GPU/VM switch: update the host's profile with `zcli update-host <hostname> <profile>` and then rebuild the host target

Validation and troubleshooting
- nix flake check for a quick sanity test
- zcli diag to produce a detailed hardware/system report
- If the repo is not under ~/ownix, update modules/core/nh.nix so zcli/nh operate on the correct path
